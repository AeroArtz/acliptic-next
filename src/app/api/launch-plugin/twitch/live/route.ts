...
async function get_m3u8_twitch_live(channelName: string) {
    try {
        const streams = await twitch.getStream(channelName);
        ...
        let streamUrl = null;
        let targetStream = streams.find(stream => 
            stream.quality.includes('480p') || 
            stream.resolution === '854x480' || 
            stream.resolution === '852x480');
